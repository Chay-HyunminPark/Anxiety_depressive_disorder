...
# Function to create a plot for a given group with specified y-axis limits
create_plot <- function(data, group_name, ymax) {
  data_group <- data %>%
    filter(Group == group_name) %>%
    group_by(Time.Period.Label, Subgroup) %>%
    summarise(Value = sum(Value), .groups = "drop") %>%
    arrange(Time.Period.Label, Subgroup)

  # Add cumulative sum of Value for label positioning
  ...
  # Create the plot
  plot <- ggplot(data_group, aes(x = Time.Period.Label, y = Value, fill = Subgroup)) +
    geom_bar(stat = "identity", position = "stack") +
    geom_text(aes(y = label_pos, label = ifelse(segment_size > 5, Subgroup, "")), 
              position = position_stack(vjust = 0.5), 
              color = "white", family = "Times", check_overlap = TRUE, 
              size = ifelse(data_group$segment_size > 5, 3, 0)) +
...
# Create plots for each group with specific y-axis limits
plot_age <- create_plot(lonely_data, "By Age", 100)
plot_sex <- create_plot(lonely_data, "By Sex", 30) 
plot_education <- create_plot(lonely_data, "By Education", 70)
plot_race <- create_plot(lonely_data, "By Race/Hispanic ethnicity", 70)

# Arrange plots in a 2 x 2 grid
grid_plots <- grid.arrange(plot_age, plot_sex, plot_education, plot_race, ncol = 2, nrow = 2)
