...
# Install the following package if wasn't installed already
...
# Load the dataset
data <- read.csv(file = here::here("inputs/data/Indicators_of_Anxiety_or_Depression_Based_on_Reported_Frequency_of_Symptoms_During_Last_7_Days_20240401.csv"))
...
aggregated_data <- filtered_data %>%
  group_by(Time.Period.End.Date, Indicator) %>%
  summarise(AverageValue = mean(Value, na.rm = TRUE))

# Plot the data
ggplot(aggregated_data, aes(x = Time.Period.End.Data, y = AverageValue, color = Indicator)) +
  geom_line() +
