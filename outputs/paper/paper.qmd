...
# Filter the dataset for the specified symptoms
filtered_data <- data %>%
  filter(Indicator == "Symptoms of Depressive Disorder")  # Assuming you need only this indicator

# Ensure that 'Time Period End Date' is a Date object
# Make sure to use the exact column name as it appears in your CSV file, including using backticks if necessary
filtered_data$`Time Period End Date` <- as.Date(filtered_data$`Time Period End Date`, format = "%Y-%m-%d")
